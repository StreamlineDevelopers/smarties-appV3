...
const Login = () => { //#[x]: @szyrelle add loading state while login
  ...
  const handleSmartiesFreePlan = async (userId) => {
    if (!userId) {
    ...
    try {
      ...
      const user = await watcher.fetchAccountDetails(userId);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      await handleSmartiesFreePlan(res.id);
    } catch (err) {
          ...
          <div className="signin-center">
            ...
            <div className="siginin-div">
              <div className="signin-formblock w-form">
                <form ref={formRef} id="wf-form-login-form" name="wf-form-login-form" data-name="login form" method="get" data-wf-page-id="682d7cdb2cd861e606be0a77" data-wf-element-id="86a1b823-6722-4912-718e-32f306ade401">
                  <FormrowItem77 textDiv="Email" name="email" dataName="Email" placeholder="Enter Email" type="email" id="email" isRequired={true} value={email} onChange={handleEmailChange} />
                  <FormrowItem77 textDiv="Password" name="password" dataName="Password" placeholder="Enter Password" type="password" id="password" isRequired={true} value={password} onChange={handlePasswordChange} />
                  <div className="form-row horizontal">
                    {/* <label className="w-checkbox remember-checkfield"><input type="checkbox" name="check-remember-account" id="check-remember-account" data-name="check remember account" className="w-checkbox-input" /><span className="checkbox-text w-form-label" htmlFor="check-remember-account">Remember for 30 days</span></label> */}
                    {/* <a href="forgot-password.html" className="link-style2">Forgot Password</a> */}
                  </div>
