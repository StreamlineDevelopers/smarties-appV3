...
class WidgetWatcher extends Watcher2 {
    ...
    sanitizeInput(data) {
        ...
        if (typeof data.assistantId === 'string' && this.isValidUUIDv4(data.assistantId.trim())) {
    ...
    async loadWidget(siteId) {
        try {
            if (siteId) {
                ...
                let script = await widgetClient.loadWidget(siteId, true);
                // script = script.replace(/http:\/\//g, "https://");
                widgetClient.runWidget(script);
            }
        } catch (error) {
